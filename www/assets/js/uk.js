...
function com_ajsoftpk_KeyPress(textbox, evt) {
    ...
    if (evt) {
        if (window.event) { keyCode = evt.keyCode; } else { keyCode = evt.charCode; }
        if (evt.ctrlKey == true) { return true; }
        if (evt.altKey == true) { return true; }
        if (keyCode == 0) { return true; }
    } else { alert("Wrong version"); return true; } { evt.returnValue = false; }
    ...
    if (com_ajsoftpk_isValidAlphabet(keyChar)) {
        ...
        if (apnaChar == keyChar) { com_ajsoftpk_replaceEndOfWord(textbox, com_ajsoftpk_findLastChar(textbox)); }
...
function com_ajsoftpk_findLastChar(textbox) {
    if (textbox.createTextRange) {
        var range;
        if (document.selection) { range = document.selection.createRange().duplicate(); } else { range = textbox.createTextRange(); }
